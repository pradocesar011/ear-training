import {
  ...
  HUNGER_MS, EGG_BASE_PRICE, EGG_MULTIPLIER, RELEASE_BASE,
...
export function calcAlgaeFeedCost(rarity, level) {
  if (rarity === 'Common')    return 1
  if (rarity === 'Rare')      return 1 + Math.ceil(level / 3)
  if (rarity === 'Legendary') return 3 + Math.ceil(level / 2)
  return 1
}
...
export function calcPendingPearls(fish) {
  ...
  const hungryAt     = lastFed + HUNGER_MS

  // Productive window: from max(lastCollected, lastFed) to min(now, hungryAt)
...
export function isHungry(fish) {
  return Date.now() > new Date(fish.last_fed_at).getTime() + HUNGER_MS
}

export function hungerRemainingMs(fish) {
  const hungryAt = new Date(fish.last_fed_at).getTime() + HUNGER_MS
