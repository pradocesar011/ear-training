  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <StatCard label="Users"          value={globalStats?.totalUsers ?? '—'}  color="text-indigo-400" />
          ...
          <div className="h-36">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={filteredNewUsers} margin={{ top: 4, right: 4, left: -20, bottom: 4 }}>
                ...
                <XAxis dataKey="date" stroke="#334155" tick={{ fill: '#475569', fontSize: 9 }}
                ...
                />
                <Bar dataKey="users" fill="#818cf8" radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </Section>
          ...
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={idmDistData} margin={{ top: 4, right: 4, left: -20, bottom: 4 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                <XAxis dataKey="idm" stroke="#334155" tick={{ fill: '#475569', fontSize: 10 }} />
                <YAxis stroke="#334155" tick={{ fill: '#475569', fontSize: 10 }} allowDecimals={false} />
                <Tooltip
                  contentStyle={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 6 }}
                  labelStyle={{ color: '#64748b', fontSize: 11 }}
                ...
                />
                <Bar dataKey="users" fill="#f59e0b" radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </Section>
        ...
        <div className="mb-4">
          <input
            ...
            className="bg-slate-900 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white
              placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 w-full max-w-xs"
          />
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {sortedUsers.map(u => (
                ...
                >
                  <td className="py-3 px-3 font-mono text-indigo-400 font-bold">{u.code}</td>
                  <td className="py-3 px-3 text-slate-400 whitespace-nowrap">
                    {u.created_at ? new Date(u.created_at).toLocaleDateString() : '—'}
                  </td>
                  <td className="py-3 px-3 text-slate-300">{u.sessions_count}</td>
                  <td className="py-3 px-3 text-slate-400 whitespace-nowrap">
                  ...
                  <td className="py-3 px-3 font-mono">
                    {u.mean_precision != null ? (
                      <span style={{
                        color: u.mean_precision >= 0.7 ? '#22c55e'
                          : u.mean_precision >= 0.4 ? '#f59e0b' : '#ef4444',
                      }}>
                  ...
                  <td className="py-3 px-3 text-slate-500 uppercase text-xs">{u.language ?? '—'}</td>
                </tr>
              ))}
              {!sortedUsers.length && (
...
export function Section({ title, children, action }) {
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-xl" style={{ padding: '20px' }}>
...
export function StatCard({ label, value, color = 'text-slate-200' }) {
  return (
    ...
    <div className="flex flex-col gap-6 animate-pulse">
      ...
      <div className="grid grid-cols-6 gap-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="h-20 bg-slate-800 border border-slate-700 rounded-xl" />
        ))}
      </div>
      <div className="grid grid-cols-3 gap-4">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="h-56 bg-slate-800 border border-slate-700 rounded-xl" />
        ))}
      </div>
