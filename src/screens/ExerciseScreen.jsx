  ...
  return (
    <div className="screen-enter flex flex-col items-center min-h-screen px-4 pt-5 gap-5">
      ...
      <div className="w-full max-w-2xl mx-auto flex flex-col gap-2">
        ...
        {/* Extra hearings row */}
        {((extraHearings ?? 0) > 0 || hearingsLeft < 2) && (
          <div className="flex items-center justify-between bg-zinc-900/60 border border-zinc-800
                          rounded-xl px-4 py-2">
            <span className="text-zinc-400 text-sm">
              Extra hearings: <span className="text-cyan-300 font-bold">{extraHearings ?? 0}</span>
            </span>
            <button
              onClick={onUseExtraHearing}
              disabled={hearingsLeft >= 2 || (extraHearings ?? 0) <= 0}
              className="px-3 py-1.5 bg-cyan-700 text-white text-xs font-semibold rounded-lg
                         hover:bg-cyan-600 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            >
              Use one
            </button>
          </div>
